...
const faixasPrecoSem: Record<string, number> = {
  "60": 329.9, "150": 369.9, "250": 409.9, "300+": 449.9,
};
const faixasPrecosCom: Record<string, number> = {
  "60": 389.9, "150": 469.9, "250": 549.9, "300+": 629.9,
};
...
function FormularioEbookImpressao() {
  ...
  const precoBase = modalidade === "sem"
    ? (faixasPrecoSem[faixa] ?? 329.9)
    : (faixasPrecosCom[faixa] ?? 389.9);
  ...
  function toggleAdicional(id: Adicional) {
    setAdicionais((prev) => { const n = new Set(prev); if (n.has(id)) n.delete(id); else n.add(id); return n; });
  }

  function handleArquivo(e: React.ChangeEvent<HTMLInputElement>, setter: (v: FileState) => void, tipos: string[], maxMb: number, label: string) {
    const file = e.target.files?.[0] ?? null;
    if (!file) return;
    const ext = file.name.split(".").pop()?.toLowerCase() ?? "";
    if (!tipos.includes(ext)) { setter({ file: null, error: `Formato inválido. Aceito: ${tipos.join(", ")}` }); return; }
    if (file.size > maxMb * 1024 * 1024) { setter({ file: null, error: `Arquivo muito grande. Máximo: ${maxMb}MB` }); return; }
  ...
  return (
    <section className="py-20 sm:py-28 bg-[var(--color-bg)]">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href={`/publicar/ebook-impressao?modalidade=${modalidade}&faixa=${faixa}`} className="inline-flex items-center gap-2 text-sm text-[var(--color-text-muted)] hover:text-cyan-400 transition-colors mb-8">
          <ArrowLeft className="w-4 h-4" />Voltar para o pacote
        </Link>
        <div className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight">Submeter obra — Ebook + Impressão</h1>
          <p className="mt-1 text-[var(--color-text-muted)]">Preencha os dados e faça upload dos arquivos para as versões digital e física.</p>
        </div>
        <AuthGate>
          <form onSubmit={handleSubmit} className="flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select id="categoria" value={categoria} onChange={(e) => setCategoria(e.target.value)} className="w-full px-4 py-2.5 rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] text-sm outline-none transition-colors focus:border-cyan-500/60 focus:ring-2 focus:ring-cyan-500/20">
                    ...
                    <option>Acadêmico</option><option>Literatura</option><option>Infantil</option><option>Tecnologia</option>
                    <option>Direito</option><option>Gestão</option><option>Saúde</option><option>Educação</option><option>Outro</option>
                  </select>
                </div>
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="resumo" className="text-sm font-medium">Resumo da obra</label>
                  <textarea id="resumo" value={resumo} onChange={(e) => setResumo(e.target.value)} rows={3} placeholder="Breve descrição (será exibida no catálogo)" className="w-full px-4 py-2.5 rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] text-sm outline-none transition-colors focus:border-cyan-500/60 focus:ring-2 focus:ring-cyan-500/20 resize-none" />
                </div>
              </div>
            </div>
            ...
            <div className="gradient-card rounded-[var(--radius-xl)] border border-[var(--color-border)] p-6">
              ...
              <label className={`flex items-center gap-4 px-4 py-4 rounded-[var(--radius-md)] border cursor-pointer transition-all mt-4 ${openAccess ? "border-cyan-500 bg-cyan-500/5" : "border-[var(--color-border)] hover:border-cyan-500/30"}`}>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-[var(--color-text-muted)] mt-1 ml-6">Se ativado, o PDF da obra será público no catálogo para download gratuito.</p>
                </div>
              </label>
            </div>
            ...
            <div className="gradient-card rounded-[var(--radius-xl)] border border-[var(--color-border)] p-6">
              ...
              <p className="text-xs text-[var(--color-text-muted)] mb-5">
                {modalidade === "com" ? "Envie o arquivo Word (.docx) para diagramação." : "Envie o arquivo já finalizado."}
                Os arquivos de capa física podem ser adaptados se necessário.
              </p>
              <div className="mb-4">
                <p className="text-sm font-medium mb-2">Arquivo da obra <span className="text-red-500">*</span> <span className="ml-2 text-xs font-normal text-[var(--color-text-muted)]">{tiposObra.join(", ").toUpperCase()} · máx. 100MB</span></p>
                {arquivoObra.file ? (
                  <div className="flex items-center gap-3 px-4 py-3 rounded-[var(--radius-md)] border border-cyan-500/30 bg-cyan-500/5">
                    ...
                    <button type="button" onClick={() => { setArquivoObra({ file: null, error: null }); if (obraRef.current) obraRef.current.value = ""; }} className="text-[var(--color-text-muted)] hover:text-red-500"><X className="w-4 h-4" /></button>
                  </div>
                ) : (
                  <button type="button" onClick={() => obraRef.current?.click()} className={`w-full flex flex-col items-center justify-center gap-2 px-4 py-6 rounded-[var(--radius-md)] border-2 border-dashed transition-colors ${arquivoObra.error ? "border-red-400 bg-red-500/5" : "border-[var(--color-border)] hover:border-cyan-500/40 hover:bg-cyan-500/5"}`}>
                    <Upload className="w-6 h-6 text-[var(--color-text-muted)]" /><span className="text-sm text-[var(--color-text-muted)]">Clique para selecionar</span>
                  </button>
                )}
                {arquivoObra.error && <p className="mt-1.5 flex items-center gap-1.5 text-xs text-red-500"><AlertCircle className="w-3.5 h-3.5" />{arquivoObra.error}</p>}
              ...
              <div>
                <p className="text-sm font-medium mb-2">Capa <span className="text-red-500">*</span> <span className="ml-2 text-xs font-normal text-[var(--color-text-muted)]">JPG, PNG · máx. 10MB</span></p>
                {arquivoCapa.file ? (
                  <div className="flex items-center gap-3 px-4 py-3 rounded-[var(--radius-md)] border border-cyan-500/30 bg-cyan-500/5">
                    <Image className="w-4 h-4 text-cyan-500 shrink-0" />
                    <span className="text-sm truncate flex-1">{arquivoCapa.file.name}</span>
                    <button type="button" onClick={() => { setArquivoCapa({ file: null, error: null }); if (capaRef.current) capaRef.current.value = ""; }} className="text-[var(--color-text-muted)] hover:text-red-500"><X className="w-4 h-4" /></button>
                  </div>
                ) : (
                  <button type="button" onClick={() => capaRef.current?.click()} className={`w-full flex flex-col items-center justify-center gap-2 px-4 py-6 rounded-[var(--radius-md)] border-2 border-dashed transition-colors ${arquivoCapa.error ? "border-red-400 bg-red-500/5" : "border-[var(--color-border)] hover:border-cyan-500/40 hover:bg-cyan-500/5"}`}>
                    <Upload className="w-6 h-6 text-[var(--color-text-muted)]" /><span className="text-sm text-[var(--color-text-muted)]">Clique para selecionar a capa</span>
                  </button>
                )}
                {arquivoCapa.error && <p className="mt-1.5 flex items-center gap-1.5 text-xs text-red-500"><AlertCircle className="w-3.5 h-3.5" />{arquivoCapa.error}</p>}
            ...
            <div className="gradient-card rounded-[var(--radius-xl)] border border-[var(--color-border)] p-6">
              ...
              <div className="flex flex-col gap-3">
                {[
                  ...
                  { id: "revisao" as Adicional, label: "Revisão ortográfica", desc: "Revisão profissional do texto", preco: `+R$ ${precoRevisao.toFixed(2).replace(".", ",")}` },
                  ...
                  { id: "kindle" as Adicional, label: "Arquivo para Kindle", desc: "Versão compatível (+5 dias úteis)", preco: "Incluído" },
                ].map((item) => (
            ...
            <div className="gradient-card rounded-[var(--radius-xl)] border border-[var(--color-border)] p-6">
              ...
              <div className="flex flex-col gap-2 text-sm">
                <div className="flex justify-between text-[var(--color-text-muted)]">
                  <span>Pacote ebook+impressão ({modalidade === "sem" ? "sem diagramação" : "com diagramação"})</span>
                ...
                {adicionais.has("revisao") && <div className="flex justify-between text-[var(--color-text-muted)]"><span>Revisão</span><span>R$ {precoRevisao.toFixed(2).replace(".", ",")}</span></div>}
                {adicionais.has("abnt") && <div className="flex justify-between text-[var(--color-text-muted)]"><span>ABNT</span><span>R$ {precoRevisao.toFixed(2).replace(".", ",")}</span></div>}
                ...
                <div className="flex justify-between font-bold text-base pt-2 border-t border-[var(--color-border)] mt-1">
                  <span>Total</span><span className="text-gradient-cyan">R$ {total.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</span>
                </div>
                <p className="text-xs text-[var(--color-text-subtle)]">Pagamento em até 5x sem acréscimo</p>
              </div>
              <button type="submit" disabled={loading} className="mt-6 inline-flex items-center justify-center gap-2 w-full px-6 py-3 rounded-[var(--radius-md)] text-sm font-semibold bg-cyan-500 hover:bg-cyan-400 disabled:opacity-60 text-black transition-all shadow-cyan">
                {loading ? "Enviando..." : <>Ir para pagamento — R$ {total.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}<ArrowRight className="w-4 h-4" /></>}
              </button>
            </div>
          </form>
        </AuthGate>
