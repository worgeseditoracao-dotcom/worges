...
import { ArrowLeft, Printer, CircleCheck, Loader2 } from "lucide-react";
import toast from "react-hot-toast";
import { AuthGate } from "@/components/AuthGate";

const faixas = [
  { label: "Até 60 páginas", value: "60", price: 289.90 },
  { label: "Até 150 páginas", value: "150", price: 339.90 },
  { label: "300 páginas ou mais", value: "300+", price: 379.90 },
];
...
export default function ImpressaoSubmeterPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-subtle)]">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
        <Link href="/publicar/impressao" className="inline-flex items-center gap-2 text-sm text-[var(--color-text-muted)] hover:text-cyan-400 transition-colors mb-8">
          <ArrowLeft className="size-4" />Voltar
        </Link>
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 rounded-[var(--radius-sm)] gradient-cyan flex items-center justify-center"><Printer className="w-5 h-5 text-black" /></div>
          <div><h1 className="text-xl font-bold">Arquivo para Impressão</h1><p className="text-xs text-[var(--color-text-muted)]">Submissão</p></div>
        </div>
